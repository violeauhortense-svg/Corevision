...
export function CompteRenduProgressif({ clientId, clientName, clientData }: CompteRenduProgressifProps) {
  ...
  const validerDossier = () => {
    ...
    if (!toutesCompletes) {
      toast.error('⚠️ Toutes les sections doivent être générées avant validation');
      return;
    }
    
    // Vérifier les dates obligatoires
    if (!dossier.dateDecouverte || !dossier.datePresentation) {
      toast.error('⚠️ Dates de découverte et présentation requises');
      return;
    }
  ...
  const sections = [
    ...
    { key: 'section4_problematiques', titre: '4. Problématiques', icon: '⚠️', color: 'orange' },
