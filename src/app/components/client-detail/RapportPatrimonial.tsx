...
export function RapportPatrimonial({ clientId, clientName }: RapportPatrimonialProps) {
  ...
  const genererRapport = async () => {
    ...
    try {
      ...
      // 1️⃣ Récupérer les données complètes du client via ClientService
      ...
      if (result.error || !result.client) {
        ...
        console.log('⚠️ Fallback : recherche dans localStorage...');
        ...
        if (!clientData) {
          console.warn('⚠️ Client introuvable - Création d\'un profil minimal');
        ...
        // 2️⃣ Envoyer les données au backend via POST
        const url = `${apiBaseUrl}/make-server-cac859af/rapport-patrimonial`;
