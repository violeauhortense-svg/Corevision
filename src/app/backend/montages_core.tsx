...
type MontageTemplateProvider = () => Omit<MontagePatrimonial, 'id' | 'date_creation' | 'date_modification'>[];

const templateProviders: Map<string, MontageTemplateProvider> = new Map();

export function registerMontageTemplates(version: string, provider: MontageTemplateProvider) {
  templateProviders.set(version, provider);
}

function getAllTemplates(): Omit<MontagePatrimonial, 'id' | 'date_creation' | 'date_modification'>[] {
  const allTemplates: Omit<MontagePatrimonial, 'id' | 'date_creation' | 'date_modification'>[] = [];
  for (const provider of templateProviders.values()) {
    allTemplates.push(...provider());
  }
  return allTemplates;
}
...
export async function getMontage(montageIdOrName: string): Promise<MontagePatrimonial | null> {
  try {
    ...
    const montageStatique = templates.find(
...
export async function updateMontage(
  ...
  try {
    ...
    const montageModifie: MontagePatrimonial = {
      ...montageExistant,
      ...updates,
      id: montageExistant.id,
      date_creation: montageExistant.date_creation,
      date_modification: new Date().toISOString(),
    };
...
/**
 * Importer des montages en masse
 */
