...
export async function generateDERLinksForCouple(
  ...
  try {
    ...
    const clientResponse = await fetch(
      `${apiBaseUrl}/make-server-cac859af/clients/${clientData.id}/der/generate-link`,
      {
        method: 'POST',
        headers: {
    ...
    if (hasSpouse && clientData.spouse) {
      ...
      if (spouseResponse.ok) {
        ...
        console.warn('⚠️ Échec génération lien conjoint, continue sans');
      }
    }
...
export async function updateTaskWithDERStatus(
  ...
  try {
    ...
    if (!storedTasks) {
      ...
      // ⚠️ Au lieu de retourner false, émettre un événement pour forcer le rechargement
...
export async function updateDERTaskByClientId(
  ...
  try {
    ...
    if (!storedTasks) {
      console.log('ℹ️ Aucune tâche trouvée pour ce client (normal si client nouveau):', tasksKey);
      return false;
    }
    ...
    if (taskIndex === -1) {
      console.warn(`⚠️ Aucune tâche DER trouvée pour le client: ${clientId}`);
      return false;
    }
