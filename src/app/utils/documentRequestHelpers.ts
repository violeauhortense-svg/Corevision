...
export async function createDocumentRequests(
  ...
  try {
    ...
    if (!task) {
      console.log(`⚠️ Tâche "Réception des documents clients" non trouvée, création au stage ${currentStage}...`);
...
export async function uploadRequestedDocument(
  ...
  try {
    ...
    if (!task || !task.documentRequests) {
      console.warn('⚠️ Aucune demande de document trouvée');
      return false;
    }
    ...
    if (docIndex === -1) {
      console.warn('⚠️ Document demandé non trouvé:', documentId);
      return false;
    }
